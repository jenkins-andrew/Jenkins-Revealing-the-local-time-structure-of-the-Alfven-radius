...
class field_models(object):
    ...
    def CAN_sheet(self, r, theta, phi):
        ...
        OUTPUT:
            B - array containing the B field components in nT in a spherical system III 
                reference frame.
        ...
        if rho_cs < a:  # inside region 1
            ...
            Bz = 2 * D / np.sqrt(z_cs ** 2 + rho_cs ** 2) - ((a ** 2) / 4) * (
                        (z_cs - D) / F1 ** 3 - (z_cs + D) / F2 ** 3)
    ...
    def SchmidtLegendre(self, n, m, theta):
        ...
        for i in range(n + 1):
            ...
            term = ((-1) ** (i)) * coeffs[i] * np.cos(theta) ** (
                        ((2 * n) - (2 * i)) - order)  # (-1)**(i) determines the sign
            terms.append(term)
    ...
    def Internal_Field(self, r, theta, phi, currentOn, model=None):
        ...
        INPUT:
            ...
            model - Internal field model, choices include:
                ...
                O4
                SHA
                simple (NOTE this is just a simple dipole)
        ...
        if model == 'JRM09':
            ...
            # This is just a very simple dipole model!
            order = 1
            g = {'1,0': (430000), '1,1': 0}
            h = {'1,0': 0., '1,1': 0}
